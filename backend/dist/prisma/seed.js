...
async function main() {
    ...
    for (const cat of categories) {
        await client_1.default.category.upsert({
            where: { code: cat.code },
            update: {},
            create: cat,
        });
    ...
    // Seed Faculty
    const facultyPassword = await bcryptjs_1.default.hash('faculty123', 12);
    await client_1.default.user.upsert({
    ...
    const admTypes = ['KCET', 'Management', 'KRLMPCA', 'Diploma'];
    ...
    const deptCode = { CSE: 'CS', ECE: 'EC', ME: 'ME', CE: 'CV', EEE: 'EE', ISE: 'IS', 'AI&ML': 'AD', CV: 'CV' };
    for (let i = 1; i <= 40; i++) {
